...
public class MotorVehicleSolved {
	 ...
	 */
	public MotorVehicleSolved(String make, String model, String licensePlate, int year, double mileage, int wy, double wm) {
		this.make = make;
		this.model = model;
	 ...
	 */
	public MotorVehicleSolved(MotorVehicleSolved mv) {
		this.make = mv.make;
		this.model = mv.model;
	 ...
	 */
	public static MotorVehicleSolved findLeastMileage(MotorVehicleSolved[] vehicles) {
		if(vehicles.length == 0) {return null;}
		MotorVehicleSolved lessMiles = vehicles[0];
	 ...
	 */
	public MotorVehicleSolved[] filterByMakeModel(MotorVehicleSolved[] inventory) {
		ArrayList<MotorVehicleSolved> filtered = new ArrayList<MotorVehicleSolved>();
		for(int i = 0; i < inventory.length; i++) {
			if(inventory[i].getMake() == this.getMake() && inventory[i].getModel() == this.getModel()) {
				filtered.add(inventory[i]);
			}
		}
		return filtered.toArray(new MotorVehicleSolved[0]);
	}
	
	/*
	 * Returns true if and only if all cars have less mileage than the target car
	 */
	public boolean allHaveLessMileage(MotorVehicleSolved[] inventory) {
	 ...
	 * inventory array.
	 */
	public MotorVehicleSolved[] buy(MotorVehicleSolved[] inventory) {
		int count = 0;
		boolean exists = false;
		for(int i = 0; i < inventory.length; i++) {
			if(inventory[i] instanceof MotorVehicleSolved) {
				count++;
